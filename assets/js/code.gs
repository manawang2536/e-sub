...
function getSheetData(ss, sheetName, useCache = true) {
  ...
  if (sheetName === "supervision_records") {
    for (let i = 1; i < values.length; i++) {
      ...
      let topicStr = "";
      try {
        const parsedSc = JSON.parse(scoresStr);
        if (parsedSc && (parsedSc._topic || parsedSc.topic)) {
          topicStr = String(parsedSc._topic || parsedSc.topic).trim();
        }
      } catch(e) {}
      if (!topicStr && row[29] !== undefined) {
        topicStr = String(row[29]).trim();
      }
...
function saveData(ss, input) {
  ...
  let scoresObj = input.scores || {};
  if (typeof scoresObj === 'string') {
    try { scoresObj = JSON.parse(scoresObj); } catch(e) { scoresObj = {}; }
  }
  if (input.topic || input.lesson) {
    scoresObj._topic = input.topic || input.lesson;
  }
...
function updateReport(ss, input) {
  ...
  let scoresObj = input.scores || (existingRow[15] ? JSON.parse(existingRow[15]) : {});
  if (typeof scoresObj === 'string') {
    try { scoresObj = JSON.parse(scoresObj); } catch(e) { scoresObj = {}; }
  }
  if (input.topic !== undefined || input.lesson !== undefined) {
    scoresObj._topic = input.topic || input.lesson || "";
  }
